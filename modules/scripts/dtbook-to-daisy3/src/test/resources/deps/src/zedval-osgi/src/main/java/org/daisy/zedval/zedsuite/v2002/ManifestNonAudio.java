...
/**
 * Performs package file manifest integrity check for non-audio book types:
 * <dl>
 * <dt>opf_mnfZeroAudio</dt>
 * <dd>Manifest does not reference audio files</dd>
 * </dl>
 * 
 * @author James Pritchett
 */

public class ManifestNonAudio extends ZedCustomTest {

	/**
	 * Performs the manifest integrity tests on the package file for non-audio
	 * book types
	 * 
	...
	public void performTest(ZedFile f) throws ZedCustomTestException {
		...
		// TEST: Manifest does not reference audio files
		if (p.getAudioFiles() != null) {
		    
			if (p.getAudioFiles().size() == 1) {
			    super.addTestFailure("opf_mnfZeroAudio","1 audio file is listed in manifest.", null, null);
			} else {
			    super.addTestFailure("opf_mnfZeroAudio",p.getAudioFiles().size()+ " audio files are listed in manifest", null, null);
			}
		}
